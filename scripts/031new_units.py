...
unitlist = pd.read_csv(RAW +'20221003_UnitList_US.csv')
...
unique = unique[['DistAlameda','BODEGA','TAMANO','RENT RATE','RR M2','min','NoUnits','TIPO','DESCRIPCION','branch_name']]
...
MUNITS1 = multiple01.groupby('CLIENT CODE')['BODEGA'].apply(lambda x:' , '.join(map(str,x)))
MUNITS2 = multiple01.groupby('CLIENT CODE')['TIPO'].apply(lambda x:' , '.join(map(str,x)))

multiple = MULTUNITS.merge(multiple01[~multiple01.index.duplicated(keep='first')][['DESCRIPCION','branch_name']]
                        ,left_index=True,right_index=True,how='left')

multiple = multiple.merge(MUNITS1.to_frame(), left_index=True, right_index=True, how='left')
multiple = multiple.merge(MUNITS2.to_frame(), left_index=True, right_index=True, how='left')

#%% Modify the column TIPO
list(multiple['TIPO'].unique())

VALS = {'Bodega Abajo , Bodega Abajo':'Bodega Abajo',
        'Down/In , Bodega Abajo':'Down/In y Bodega Abajo',
        'Bodega Arriba , Bodega Arriba , Bodega Arriba , Bodega Abajo':'Bodega Arriba y Bodega Abajo',
        'Bodega Arriba , Bodega Arriba':'Bodega Arriba',
        'Bodega Abajo , Bodega Arriba':'Bodega Arriba y Bodega Abajo',
        'Bodega Abajo , Bodega Abajo , Bodega Abajo , Bodega Abajo , Bodega Abajo , Bodega Arriba , Bodega Arriba , Bodega Arriba':'Bodega Arriba y Bodega Abajo',
        'Bodega Abajo , Down/In':'Down/In y Bodega Abajo',
        'LOCKER ARRIBA , LOCKER ARRIBA':'LOCKER ARRIBA',
        'Bodega Abajo , Bodega Abajo , Bodega Abajo':'Bodega Abajo',
        'Bodega Abajo , Bodega Arriba , Down/In':'Bodega Abajo , Bodega Arriba y Down/In'}

multiple = multiple.replace({'TIPO':VALS})
...
for SUC in sucursales:
    ...
    UNITSDF = UNITSDF[['SUCURSAL','BODEGA','TAMANO','TIPO','DESCRIPCION','RENT RATE']]
    # Clients df
    CLIENTSDF = dic_multclient[SUC].reset_index()
    CLIENTSDF = CLIENTSDF[['CLIENT CODE','BODEGA','TAMANO','TIPO','DESCRIPCION','RENT RATE']]
...
# =============================================================================
#%% 6.0 - Clients with a single unit
# =============================================================================
...
remainingclients = remainingclients[['BODEGA','TAMANO','TIPO','RENT RATE']]
...
clientsdf = unique[['BODEGA','TAMANO','TIPO','DESCRIPCION','RENT RATE']]
...
clients_new_units_02 = ok02.copy()

# =============================================================================
#%% 6.1 - Clients with a single unit and exact match on type
# =============================================================================
...
for ID in remainingclientsid:
    ...
    for ORDER in aaorder:
        ...
        for ROW in range(len(UNITSDF)):
            ...
            if (remainingclients.loc[ID,'TAMANO'] <= UNITSDF.loc[ROW,'TAMANO'] <= remainingclients.loc[ID,'TAMANO']*1.25) and (remainingclients.loc[ID,'RENT RATE']*.8 <= UNITSDF.loc[ROW,'RENT RATE'] <= remainingclients.loc[ID,'RENT RATE']*1.2) and (remainingclients.loc[ID,'TIPO'] == UNITSDF.loc[ROW,'TIPO']):
                CLIENTID.append(remainingclients.loc[ID,'ClientID'])
                ACTUALUNIT.append(remainingclients.loc[ID,'BODEGA'])
                NEWBRANCH.append(UNITSDF.loc[ROW,'SUCURSAL'])
                BRANCHORDER.append(ORDER)
...
clients_new_units_all_options03 = ok.copy()
...
#%% Preffered 
# =============================================================================
clients_tipo_match = clients_new_units_03[clients_new_units_03['Proximity'] < 1]

clients_new_units_00 = pd.concat([clients_tipo_match,clients_new_units_02],axis=0)
clients_new_units_00.drop_duplicates(subset='ClientID',keep='first',ignore_index=True,inplace=True)
...
COLS = {'BODEGA_x':'Bodega actual', 'TAMANO_x':'Tamaño actual',
        ...
        'DESCRIPCION_y':'Descripción nueva', 'RENT RATE_y':'RR Nuevo','TIPO_x':'Tipo actual','TIPO_y':'Tipo nuevo'}

clients_new_units_01 = clients_new_units_01.rename(columns=COLS)
clients_new_units_00 = clients_new_units_00.rename(columns=COLS)

COLS = ['ClientID','Bodega actual','Nueva Sucursal','Proximity', 'Bodega nueva','Tamaño actual',
        'Tamaño nuevo','Tipo actual','Tipo nuevo','Descripción actual','Descripción nueva','RR actual','RR Nuevo']

clients_new_units_01 = clients_new_units_01[COLS]
clients_new_units_00 = clients_new_units_00[COLS]

newunits = pd.concat([clients_new_units_01,clients_new_units_00])

### All options for all clients
clients_new_units_all_options01['Proximity'] = '0'
...
clients_new_units_all_options01 = clients_new_units_all_options01.rename(columns=COLS)
clients_new_units_all_options02 = clients_new_units_all_options02.rename(columns=COLS)
clients_new_units_all_options03 = clients_new_units_all_options03.rename(columns=COLS)

COLS = ['ClientID','Bodega actual','Nueva Sucursal','Proximity', 'Bodega nueva','Tamaño actual',
        'Tamaño nuevo','Tipo actual','Tipo nuevo','Descripción actual','Descripción nueva','RR actual','RR Nuevo']

clients_new_units_all_options01 = clients_new_units_all_options01[COLS]
clients_new_units_all_options02 = clients_new_units_all_options02[COLS]
clients_new_units_all_options03 = clients_new_units_all_options03[COLS]

newunitsall = pd.concat([clients_new_units_all_options01,clients_new_units_all_options02])
...
newunitsexact = pd.merge(clients_new_units_all_options03,info,left_on='ClientID',right_on='CLIENT CODE',how='left')
newunitsexact.drop('CLIENT CODE',axis=1,inplace=True)
newunitsexact.reset_index(drop=True,inplace=True)
...
for IX in range(len(newunits)):
    ...
    TIPODEBODEGA = newunits.loc[IX,'Tipo actual']
    NUEVOTIPODEBODEGA = newunits.loc[IX,'Tipo nuevo']
    ...
    newunits.loc[IX,'Mensaje'] = f'Estimado {NOMBRE},\nPara nosotros en FIBRA Storage es muy importante brindarle todo nuestro apoyo en este proceso en el que nos encontramos con la sucursal de Alameda.\
        ...
        \n\
        \nCon base en la información que nos proporcionó cuando comenzó a rentar con nosotros, hemos buscado espacios disponibles en otras sucursales que están lo más cercano posible a la dirección registrada.\
        \nA continuación, le enlistamos opciones lo más parecidas al espacio de {TIPODEBODEGA} que actualmente renta en la sucursal Alameda:\
...
for IX in range(len(newunitsall)):
    ...
    TIPODEBODEGA = newunitsall.loc[IX,'Tipo actual']
    NUEVOTIPODEBODEGA = newunitsall.loc[IX,'Tipo nuevo']
...
newunitsexact['Mensaje'] = ''

for IX in range(len(newunitsexact)):
    NOMBRE = newunitsexact.loc[IX,'CLIENT NAME']
    NUEVASUCURSAL = newunitsexact.loc[IX,'Nueva Sucursal']
    TAMANO = newunitsexact.loc[IX,'Tamaño nuevo']
    NEWRR = newunitsexact.loc[IX,'RR Nuevo']
    TIPODEBODEGA = newunitsexact.loc[IX,'Tipo actual']
    NUEVOTIPODEBODEGA = newunitsexact.loc[IX,'Tipo nuevo']
    
    newunitsexact.loc[IX,'Mensaje'] = f'Estimado {NOMBRE},\nPara nosotros en FIBRA Storage es muy importante brindarle todo nuestro apoyo en este proceso en el que nos encontramos con la sucursal de Alameda.\
        \nSabemos que el dejar de contar con un espacio adicional con el que ya cuenta, es una situación no deseable. Por lo anterior, queremos ofrecerle opciones para tratar de ayudar en este proceso.\
        \n\
        \nCon base en la información que nos proporcionó cuando comenzó a rentar con nosotros, hemos buscado espacios disponibles en otras sucursales que están lo más cercano posible a la dirección registrada.\
        \nA continuación, le enlistamos opciones lo más parecidas al espacio de {TIPODEBODEGA} que actualmente renta en la sucursal Alameda:\
...
finaldic = {'Primera Opcion':newunits,'Todas las opciones':newunitsall,'Opciones mismo tipo de bodega':newunitsexact}
...
save_xls(finaldic,f'{fecha_y_hora}_{name}.xlsx')
